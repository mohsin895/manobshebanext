...
export function ComponentFourth() {
  const { t } = useLanguage()
  return (
    <div className='group w-[100%] shrink-0 snap-start sm:w-full sm:shrink sm:min-w-0' data-card>
      <div>
        <img src='/images/scholarship/exam4.png' alt='scholarship result' width={427} height={382} className='w-full aspect-[427/382] object-cover' />
      </div>
      <div className=' mt-[-38px]'>
        ...
        <div className='relative z-10 mt-[-130px]  gap-3 p-[30px] md:p-5 text-left transition-all duration-300 ease-out group-hover:items-left group-hover:text-left'>
          <div className='mt-[-30px]'>
            <h3 className='font-bn text[16px] md:text-[24px] font-medium leading-[20px] md:leading-[32px]text-[#3B3BC7]'>{t('scholarship.merit_list')}</h3>
            <p className=' font-bn-serif text-[14px] md:text-[16px] font-normal leading-[24px] text-[#3B3BC7] line-clamp-2'>{t('scholarship.merit_list_desc')}</p>
          </div>

          <div className='flex h-full justify-end items-end mt-[30px]'>
            <a href='/meritorious-student' className='inline-flex text-[12px] items-center justify-center gap-2 rounded-[99px] bg-[#4A4DE1] px-4 py-2 text-white'>
